...
function drawProvidersChart($jsonObj, controlID, subcontrols, fldLabName, fldValue, providerName, strtYr, endYr, index, Min, Max, Units, seriescolors, chartTypeCode) {
    ...
    if (fldnames.indexOf('AGPUMPED') > -1 && providerName.reg == undefined) {
        fldnames = fldnames.replace(",AGPUMPED", "");

        Max = 50;

    }
    // Sampson EDIT Begin 01.18.17 Remove AgLCLU demand if not regional
    if (fldnames.indexOf('AGDEM') > -1 && providerName.reg == undefined) {
        fldnames = fldnames.replace(",AGDEM", "");

        Max = 50;

    }
    // Sampson End Edit 01.18.17
    ...
    $.each(flds, function () {
        ...
        if (subcontrols[this] != "" && subcontrols[this] != undefined)
