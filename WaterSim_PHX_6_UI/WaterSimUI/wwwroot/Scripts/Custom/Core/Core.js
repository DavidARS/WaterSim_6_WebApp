...
// 01.01.17 - version 23.1.0 [das] new interface for WaterSim 6
// 01.19.17 - version 23.1.1 [das] added effluent to the Power Plant
// 01.27.17 - version 23.1.2 [das] added the RCP6.8 button for climate (removed moderate)
// 06.08.17 - version 24.2.0 [das] Shuchir's additions [new front page, data download capability, comparison 
//                                 of simulations, alpha and beta model tabs]
// 06.12.17 - version 25.1.0 [das] Shuchir's change of the input tab included (moved around the controls)
// 06.21.17 - version 25.2.0 [das] Shuchir's changes to the compare outputs tab (and...?)
// 10.17.17 - version 25.2.1 [das] Added a call to function setDefaultDrought() {} in Core.js
// -----------------------------------------------------------------------------------------------------------
// UI Version
var WaterSimVersion = "UI: 25.2.1  ";
...
$(document).ready(function () {
    ...
    $("#loadBSbutton").click(function () {
        ...
        if (typeof (Storage) !== "undefined") {
            ...
            SetFlowLabels();
            // 01.18.17 DAS: set the UI and model back to "strategic"
            $("#scenarioList").val("7");
            // QUAY EDIT 6/13/14
            //callWebService(getJSONData('parent'));
            ...
            //
            setDefaultDrought();
            // SAMPSON EDIT 06.29.17
            setDefaultClimate();
            // END SAMPSON EDIT 06.29.17
...
//SXI1211 
//03/22/2017 
//added middle class identifier to separate .ddlflds from simulation compare screen controls
$(".middle .ddlflds").change(function () {
    drawChart($(this).closest('div[id*=OutputControl]').attr('id'), $jsonObj);
});
...
function callWebService(jsonData) {
    ...
    $.ajax({
        ...
        success: function ($res) {
            ...
            /*SXI1121 BeginRegion Edit for multi model simulation
            5 Feb 2017*/
            saveModelData(WS_RETURN_DATA);
            /*SXI1211 EndRegion*/
...
function getJSONData(inputType) {
    ...
    if (inputType == 'empty') {
        ...
        outputFields.push(eyr["FLD"]);
        //
        //// get scenario chosen (David Iwaniec scenario)
        // 01.02.2017
        // create object
        eyr = {};
        // assign field and value
...
var seriesThree = ["#7cb5ec", "#f7a35c", "#009900"];
...
var seriesColumn3 = ["#3399FF", "#009900", "#000000"];

var seriesColumn7 = ["#E68AE6", "#FF6600", "#000000", "#88C6ED", "#009900", "#3399FF", "#FF0000"];
//                              black           orange
var seriesColumn8 = ["#ea07ea", "#FF6600", "#000000", "#88C6ED", "#009900", "#3399FF", "#FF0000", "#90ee7e"];
var qualitative = ["#66c2a5", "#fc8d62", "#8da0cb"];
var sequential_4 = ["#bdc9e1", "#67a9cf", "#1c9099", "#016c59"]
var diverging_5 = ["#d7191c", "#fdae61", "#ffffbf", "#abdda4", "#2b83ba"];
var qualitative_5 = ["#e41a1c", "#377eb8", "#4daf4a", "#984ea3", "#ff7f00"];
//
ColorSeriesArray[0] = sequential_4;
...
ColorSeriesArray[6] = seriesColumn3;
//ColorSeriesArray[7] = seriesColumn3;
ColorSeriesArray[7] = seriesColumn7;
ColorSeriesArray[8] = seriesColumn8;
ColorSeriesArray[9] = qualitative;
ColorSeriesArray[10] = qualitative_5;
...
(
    function () {
        ...
        //});
        // SAMPSON BEGIN EDIT 06.29.17
        $("#OpeningDialog").dialog({ autoOpen: false });
        ...
        $('#OpeningDialog').dialog({
            ...
            width: 700,
            resizable: false,
            dialogClass: 'OpeningDialog-dialog',
        ...
        if ($.urlParam("CAP") == "true") {
            $("#OpeningDialog").dialog("open");
        }
        // SAMPSON EDIT END 06.29.17
        // sampson edit 10.17.17
        setDefaultDrought();
        // end sampson edit 10.17.17
...
/*  SXI1211 #BeginRegion
    ...
    Description:    This method is invoked from function callWebService() in Core.js.
                    The purpose is to save the model input and output data in hidden fields 
                    for reuse without actually invoking the web service when tabs change.
    */
function saveModelData(data) {
    var activeTab = $('#alphaModelActive').val() === 'Active' ? 'alpha' : $('#betaModelActive').val() === 'Active' ? 'beta' : '';
    //debugger;
    var selectScenario = $('#scenarioList').val().toString();
    switch (activeTab) {
        case 'alpha':
            $('#alphaModelData').val(JSON.stringify(data));
            $('#alphaModelScenario').val(selectScenario);
            break;
        case 'beta':
            $('#betaModelData').val(JSON.stringify(data));
            $('#betaModelScenario').val(selectScenario);
            break;
    }

    //When both models are run atleast once, make the model compare tab visible.
    if ($('#alphaModelData').val().length > 0 && $('#betaModelData').val().length > 0) {
        $('.simulationModelCompare').attr("style", "display:block");
    }
    else {
        $('.simulationModelCompare').attr("style", "display:none");    
    }
    /*SXI1211 #EngRegion*/
}
// E.O.F.